...
def test_handle_snapshots_crew_agent_overrides(monkeypatch):
    ...
    (the canonical identity), not on the kiro ``agent`` name it also carries
    for logs."""
    from kiro_crew.config.loader import KiroCrewAgentConfig
    ...
    handle = AcpSessionHandle(
        "s1", asyncio.Queue(), rt, agent="pr-reviewer-kiro", crew_agent="pr-reviewer"
    )
    assert handle._watchdog.tool_stall_suspect_secs == 450.0
    assert handle._watchdog.agent_override is True
    # The kiro name alone must not resolve: same construction without the
    # canonical identity snapshots the globals.
    bare = AcpSessionHandle("s2", asyncio.Queue(), rt, agent="pr-reviewer-kiro")
