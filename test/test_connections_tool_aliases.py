...
def test_an_unwritable_record_raises_instead_of_swallowing(tmp_path, monkeypatch):
    """Invariant 5: a failed record write is LOUD.

    ``atomic_write`` leaves the destination untouched when it fails, so the OLD
    record survives and now describes an earlier emission than the spec carries --
    the overstating direction invariant 2 forbids. Swallowing that is the one
    remaining path to deleting a user's hand-written alias, so the failure raises.
    """
    monkeypatch.setenv("KIROCREW_HOME", str(tmp_path / "home"))
    with patch(
...
def test_a_failed_write_leaves_the_earlier_record_claiming_its_own_pairs(
    tmp_path, monkeypatch
):
    """The reason the raise is required, stated as state rather than as policy.

    Mutation-check for the swallow: restoring ``except OSError: log`` in
    ``store_emitted_aliases`` leaves this record on disk with NO error reaching the
    caller, which is precisely the silent stale claim. The assertion below is what
    the caller would then have no way to learn.
    """
    monkeypatch.setenv("KIROCREW_HOME", str(tmp_path / "home"))
    first = frozenset({("linear", "list_projects", "linear_list_projects")})
    store_emitted_aliases(first)

    # The declaration is withdrawn, so this pass emits nothing and tries to
    # relinquish the pair -- and the write fails.
    with patch(
        "kiro_crew.connections.alias_record.atomic_write", side_effect=OSError("read-only")
    ):
    ...
    # The old record survived the failure and still claims the pair. Loud failure is
    # what stops the next pass acting on this.
    assert load_emitted_aliases() == first


def test_a_record_write_failure_surfaces_out_of_the_real_rebuild(tmp_path, monkeypatch):
    """The raise reaches the rebuild's caller instead of dying inside the pass.

    Same scaffolding as the ordering test below, and the same deterministic stand-in
    for the resolver -- but the REAL ``store_emitted_aliases`` runs, with its write
    failing. Two properties in one: the failure propagates, and the spec is already
    on disk when it does (invariant 2), so what surfaces is stale bookkeeping over a
    correct spec rather than a bad spec.
    """
    ...
    emitted = frozenset({("linear", "list_projects", "linear_list_projects")})
    monkeypatch.setattr(agent_mod, "_apply_connection_tool_aliases", lambda config: emitted)
    # Only the RECORD write fails: the spec goes through agent._atomic_json_write,
    # which is a different binding, so this isolates the failure to bookkeeping.
    monkeypatch.setattr(alias_record, "atomic_write", _raise_read_only)

    with pytest.raises(OSError, match="read-only"):
        agent_mod.rebuild_agent_config()

    assert spec_path.exists(), "the spec must be durable before the record is attempted"
    assert not record_path().exists(), "the record write is the step that failed"
...
def test_the_whole_record_sequence_runs_under_the_lock_that_guards_the_spec(
    ...
    * A record READ taken before the lock is acquired is the same race from the
    ...
    depth_at: dict = {}
    seen: dict = {}
    emitted = {("linear", "list_projects", "linear_list_projects")}
    real_load = alias_record.load_emitted_aliases
    ...
    def _fake_pass(config):
        # Stands in for the resolver so the emitted set is deterministic, but does
        # the real record READ, at the real position: cleanup is authorized by
        # that record, so WHERE it is read is the property under test.
        alias_record.load_emitted_aliases()
        return emitted

    def _tracked_write(target, data):
        real_write(target, data)
        if Path(target) == spec_path:
            events.append("spec")
            depth_at["spec"] = held["depth"]

    def _observe(pairs):
        events.append("record")
        depth_at["record"] = held["depth"]
        seen["pairs"] = set(pairs)

    monkeypatch.setattr(bridges, "_mcp_lock", _tracking_lock)
    monkeypatch.setattr(alias_record, "load_emitted_aliases", _tracked_load)
    ...
    monkeypatch.setattr(alias_record, "store_emitted_aliases", _observe)

    agent_mod.rebuild_agent_config()

    assert seen.get("pairs") == emitted, "the record write never ran"
    # One critical section: every step of the sequence sees the lock held.
    ...
    assert depth_at.get("record", 0) > 0, "record written after the spec lock was released"
    # And in order, with the record immediately after the spec write: nothing
    # between them can widen the crash window the ordering exists to keep narrow.
    assert events[: events.index("record") + 1] == ["read", "spec", "record"]
