...
def rebuild_agent_config(*, clean: bool = False) -> Path:
    ...
    def _finalize_and_write() -> None:
        ...
        # Resolve collisions HERE rather than earlier, because the record this
        # pass loads to authorize cleanup must be read in the same critical
        # section that writes the spec and the new record. Computing it outside
        # the lock lets an overlapping rebuild read this pass's spec before its
        # record lands, classify these generated aliases as the user's (no record
        # claims them yet), retain them, and then write a record of its own that
        # does not name them -- and since only a record authorizes removal, they
        # can never be cleaned up again. It also means the pass sees the FINAL
        # server map, including an app registration merged in above.
        emitted_tool_aliases = _apply_connection_tool_aliases(config)
        _atomic_json_write(path, config)
        # ONLY now, with the spec carrying those aliases durably on disk, record
        # what was emitted -- and inside whatever lock guarded that write, so the
        # two land as one unit. The record authorizes the NEXT pass to delete
        # pairs, so it must never name a pair the spec does not have: writing it
        # before the spec would let a crash here leave a claim on a name the user
        # may later hand-write, and that user's entry would then be silently
        # deleted. Crashing in THIS window instead leaves an emitted-but-unrecorded
        # alias, which is unclaimable and is cleaned up one cycle later. Recording
        # outside the lock reaches that same deletion by another route: two
        # rebuilds serialize their spec writes, but their record writes could still
        # land in the opposite order, leaving a record that describes the OTHER
        # pass's spec and so claims a name this one never wrote. ``None`` means the
        # pass did not run, so the existing record stays untouched; an EMPTY set is
        # written, because that is how the pass relinquishes pairs it no longer
        # emits. A write failure here PROPAGATES on purpose (alias_record invariant
        # 5): the failed write leaves the OLD record in place, which may still claim
        # a pair this pass stopped emitting, and swallowing that is what would let a
        # later rebuild strip a name the user has since hand-written. The spec is
        # already durable above, so the surfaced error reports stale bookkeeping over
        # a correct spec.
