    ...
    def create_provider_factory(self) -> Callable:
        ...
        def _acp(
            ...
            **_kwargs: object,
            ...
            # windows on the handle). The dashboard resolves and passes it
            # explicitly (the slot member the human picked). When absent, the
            # surface convention documented on _resolve_model_for_agent applies:
            # Slack threads, cron jobs and spawned agents pass a CREW name as
            # ``agent``, so crew-namespace membership makes it canonical — this
            # is a membership check on names the surface owns, not the removed
            # cross-namespace match (the dashboard, the only kiro-name-passing
            # surface, never reaches this fallback).
            if crew_agent is None:
                crew_agent = agent if agent and agent in self.agents else ""
