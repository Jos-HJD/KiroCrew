...
class TestTheNameAloneIsNotOwnership:
    """A global entry under an opt-in name may be the user's own.

    Kiro Crew purges its managed server names from the user's global
    ``mcp.json`` because it never writes them there. That reasoning holds for
    always-on servers; for a set granted BY HAND it does not, so the entry has
    to prove Kiro Crew wrote it before it is deleted.
    """

    def test_an_opt_in_name_is_not_purged_by_name_alone(self) -> None:
    ...
    def test_a_user_written_entry_is_not_ours(self) -> None:
        theirs = {"command": "node", "args": ["/home/me/my-dashboard-server.js"]}
        assert mcp_cleanup._invokes_managed_subcommand(DASH_SERVER, theirs) is False

    def test_the_subcommand_alone_is_not_ownership(self) -> None:
        """Argv is half the test; a foreign command with our argv is still theirs."""
        theirs = {"command": "node", "args": ["mcp-dashboard"]}
        assert mcp_cleanup._invokes_managed_subcommand(DASH_SERVER, theirs) is False

    def test_the_command_alone_is_not_ownership(self) -> None:
        """And our binary running something else is not this server either."""
        other = {"command": "/usr/local/bin/kirocrew", "args": ["gateway"]}
        assert mcp_cleanup._invokes_managed_subcommand(DASH_SERVER, other) is False

    def test_our_own_entry_is_recognised(self) -> None:
        ours = {"command": "/usr/local/bin/kirocrew", "args": ["mcp-dashboard"]}
        assert mcp_cleanup._invokes_managed_subcommand(DASH_SERVER, ours) is True

    def test_the_module_form_is_recognised(self) -> None:
        """The fallback invocation Kiro Crew emits when the script is unresolved."""
        ours = {
            "command": "/usr/bin/python3.12",
            "args": ["-m", "kiro_crew", "mcp-dashboard"],
        }
        assert mcp_cleanup._invokes_managed_subcommand(DASH_SERVER, ours) is True

    def test_a_windows_script_name_is_recognised(self) -> None:
        ours = {"command": r"C:\\Python\\Scripts\\kirocrew.exe", "args": ["mcp-dashboard"]}
        assert mcp_cleanup._invokes_managed_subcommand(DASH_SERVER, ours) is True

    def test_a_non_dict_entry_is_not_ours(self) -> None:
        assert mcp_cleanup._invokes_managed_subcommand(DASH_SERVER, "broken") is False
