...
class AcpSessionHandle:
    ...
    def __init__(
        ...
        # ``agent`` is the bound kiro name the runtime spawned with — a
        # DIFFERENT namespace, kept for logs/row-store joins only. Neither name
        # may EVER become an OTel metric attribute (free-form => cardinality
        # bomb; see metrics/schema.py) — telemetry carries the agent_override
        # BOOLEAN. An explicit ``watchdog`` (tests) always wins verbatim.
        self._agent = agent
    ...
    def rebind_watchdog(self, crew_agent: str) -> None:
        ...
        """
        self._crew_agent = crew_agent
        self._watchdog = _load_watchdog_settings(crew_agent)
