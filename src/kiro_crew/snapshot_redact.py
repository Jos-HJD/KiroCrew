    ...
    try:
        with closing(sqlite3.connect(str(path))) as conn:
            ...
            if hits:
                ...
                conn.commit()
                # REWRITE the file, because an UPDATE only replaces the row -- it does not
                # erase the bytes the old row occupied. SQLite leaves the previous cell
                # content in the page's unused space, so a database whose every row reads
                # redacted can still hand the plaintext credential to anyone who greps the
                # FILE. That is the only form that matters for a copy leaving the host.
                # `VACUUM` rebuilds the database from its live content, which is exactly
                # the set that has just been cleaned. Safe here because this is a throwaway
                # egress copy, never the operator's own archive.
                conn.execute("VACUUM")
                report.replacements[rel] = report.replacements.get(rel, 0) + hits
                if fts_tables:
