...
def _contract_path(worktree: str, contract: str, name: Optional[str]) -> str:
    """Resolve a profile-declared contract, refusing anything outside the worktree.

    The contract is named by the profile, and a repo-root ``.prepare-pr.toml``
    supplies it - so an absolute path discards ``worktree`` entirely and a ``..``
    segment or a symlink walks out of it. Whatever this opens is read into the
    reviewer brief, so an escape puts an unrelated file's contents in front of a
    model. Resolution happens before the containment test so symlinks cannot
    launder the path.
    """
    target = os.path.realpath(os.path.join(worktree, contract))
    root = os.path.realpath(worktree)
    if not target.startswith(root + os.sep):
        raise ParityError(
            "the profile points the {} reviewer's contract at {!r}, which resolves "
            "outside this worktree. Refusing to read it into a brief.".format(name, contract)
        )
    if not os.path.isfile(target):
        raise ParityError(
            "the profile names {} as the {} reviewer's contract but that file "
            "does not exist in this checkout.".format(contract, name)
        )
    return target
...
def assemble(
    ...
    for reviewer in reviewers:
        ...
        contract_path = _contract_path(worktree, contract, name)
