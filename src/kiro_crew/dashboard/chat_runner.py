        ...
        async with _eager_spawn_sem:
            ...
            try:
                ...
                _, is_new, resumed = await sessions.get_or_create(
                    ...
                    # Canonical crew identity (the member the human picked) —
                    # resolved HERE, where it is owned, and plumbed to the
                    # session so per-agent watchdog windows never depend on a
                    # cross-namespace name match. "" is authoritative: this
                    # slot has no crew, so no override applies.
                    crew_agent=slot.agent or "",
