...
`tools` is gated with the entry. The shipped `defaults.json` grants
`@kirocrew-computer` unconditionally, and kiro-cli mounts a server *because*
something references it, so a surviving ref would keep spawning the very backend
the gate declined. `agent._prune_gated_managed_refs()` strips both the bare ref and
any `@kirocrew-computer/<tool>` grant. Pinned by
