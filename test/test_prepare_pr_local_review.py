...
def test_contract_path_cannot_escape_the_worktree(tmp_path):
    """The contract is named by a repo-root .prepare-pr.toml and whatever it opens
    is read into a model brief, so an escape leaks an unrelated file."""
    root = tmp_path / "wt"
    (root / ".github" / "workflows").mkdir(parents=True)
    outside = tmp_path / "secret.yml"
    outside.write_text("secret: yes\n", encoding="utf-8")
    for bad in [str(outside), "../secret.yml", ".github/../../secret.yml"]:
        with pytest.raises(local_review.ParityError) as exc:
            local_review._contract_path(str(root), bad, "gpt")
        assert "outside this worktree" in str(exc.value)
    # A symlink pointing out is resolved before the containment test.
    link = root / ".github" / "workflows" / "sneaky.yml"
    link.symlink_to(outside)
    with pytest.raises(local_review.ParityError) as exc:
        local_review._contract_path(str(root), ".github/workflows/sneaky.yml", "gpt")
    assert "outside this worktree" in str(exc.value)


def test_contract_path_inside_the_worktree_resolves(tmp_path):
    root = tmp_path / "wt"
    (root / ".github" / "workflows").mkdir(parents=True)
    real = root / ".github" / "workflows" / "codex-review.yml"
    real.write_text("on: pull_request\n", encoding="utf-8")
    got = local_review._contract_path(str(root), ".github/workflows/codex-review.yml", "gpt")
    assert Path(got) == Path(os.path.realpath(real))


def test_absent_contract_still_reports_missing(tmp_path):
    root = tmp_path / "wt"
    root.mkdir()
    with pytest.raises(local_review.ParityError) as exc:
        local_review._contract_path(str(root), ".github/workflows/gone.yml", "gpt")
