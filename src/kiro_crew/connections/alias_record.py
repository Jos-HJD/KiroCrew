...
5. **A record write that FAILS is LOUD.** Swallowing it was the last remaining
   path to deleting a name the user wrote by hand, because a failed write does
   NOT degrade to the empty record of invariant 4.
   :func:`~kiro_crew.atomic_write.atomic_write` publishes through a temp file and
   a rename, and on failure it removes the temp and leaves the DESTINATION as it
   was -- so the previous record survives, now describing an OLDER emission than
   the spec on disk carries. That is the overstating direction invariant 2
   forbids: the stale record can still claim a pair this pass stopped emitting,
   so the next rebuild strips that pair, and a user who has since hand-written
   that exact name loses it.

   The writer cannot distinguish the harmless outcome (nothing on disk, claims
   nothing) from the harmful one (a stale record survived), so it treats every
   failure as the harmful one and raises. This cannot damage the spec: by
   invariant 2 the spec write is already durable before this runs, so what
   surfaces is a bookkeeping failure over a spec that is complete and correct on
   disk -- an unwritable data home reported at the moment it happens, instead of
   a silent deletion one rebuild later.
...
def store_emitted_aliases(emitted: Collection[EmittedAlias]) -> None:
    ...
    Best-effort is NOT available here (invariant 5): a failure leaves the previous
    record in place, describing an older emission than the spec now carries, so it
    is logged and re-raised rather than swallowed.

    Raises:
        OSError: the record could not be persisted. The spec is already durable at
            this point, so the caller is being told its bookkeeping is stale -- not
            that the rebuild produced a bad spec.
    """
    payload = {
    ...
    try:
        ...
        # Re-raised, not swallowed: the old record survived this failure and now
        # over-claims, which is the one direction that can delete a user's alias.
        logger.error(
            "Could not persist the Connections alias record at %s. The record on "
            "disk still describes an earlier emission, so it may claim a tool "
            "alias this rebuild no longer writes -- failing loudly instead of "
            "letting a later rebuild strip that name.",
            path,
        )
        raise
